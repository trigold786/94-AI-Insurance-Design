...
**Goal:** Build complete 支付宝小程序 MVP with 7 pages (P1-P7) + API integration

**Architecture:** Native Alipay Mini Program (AXML+ACSS+JS), 7 independent pages under `pages/`, API layer in `services/api.js`. Navigation flows login → index → city-picker → profile → loading → preview → plan. Data flows via `app.globalData`.

**Tech Stack:** AXML, ACSS, JavaScript, Alipay Mini Program SDK (my.* API), existing backend REST API. Direct translation from WeChat Mini Program with `wx.*` → `my.*` API mapping.
...
<view class="container">
  <view class="header">
    <view class="logo-placeholder">
      <text class="logo-text">社保</text>
    </view>
    <text class="title">AI社保智筹</text>
    <text class="subtitle">AI驱动的社保规划助手</text>
  </view>
  ...
  <view class="policy-card">
    ...
    <text class="policy-link" onTap="onViewPrivacy">《隐私政策》</text>
    <text class="policy-text">和</text>
    <text class="policy-link" onTap="onViewTerms">《用户协议》</text>
  </view>

  <button class="btn-primary {{!agreed ? 'btn-disabled' : ''}}" 
          disabled="{{!agreed}}" 
          onTap="onLogin">
    <text a:if="{{!loading}}">支付宝一键登录</text>
    <text a:else>登录中...</text>
  </button>
...
1. `pages/index/` — P2 Home with city bar, hero CTA, policy scroll (use `onTap`, `a:if`, `my.navigateTo`, `my.request`)
2. `pages/city-picker/` — P3 City picker with city list, `my.navigateBack`
3. `pages/profile/` — P4 Multi-step form (3 steps), `my.request` for API
4. `pages/loading/` — P5 Loading spinner, `my.request` for generatePlan
5. `pages/preview/` — P6 Preview with blurred values
6. `pages/plan/` — P7 Plan detail with scheme comparison

Each page's WXML → AXML, WXSS → ACSS, JS API mapping as above.
...
Expected: 7 page dirs × 3 files each + app.js, app.json, app.acss, utils/constants.js, services/api.js

- [ ] **Step 2: Verify navigation flow**

login → index → city-picker → profile → loading → preview → plan — all using `my.navigateTo`/`my.redirectTo`

- [ ] **Step 3: Verify API mapping**

All `wx.request` → `my.request`, `wx.showToast` → `my.showToast` (with `content` param), etc.
