...
func TestCalculatePlanCashflowForMale(t *testing.T) {
	...
	for _, s := range resp.Schemes {
		...
		// Male retirement at 60, starting from 40 = 20 years
		if len(s.Cashflow) != 20 {
			t.Errorf("scheme %s: expected 20 cashflow years (40->60), got %d", s.Name, len(s.Cashflow))
		}
	}
}
...
func TestCalculatePlanCashflowForFemale(t *testing.T) {
	...
	// Female retirement at 55, starting from 30 = 25 years
	for _, s := range resp.Schemes {
		if len(s.Cashflow) != 25 {
			t.Errorf("scheme %s: expected 25 cashflow years, got %d", s.Name, len(s.Cashflow))
		}
	}
}
