...
**Architecture:** Level 1 (fast discovery + keyword pre-filter in Fetch()) → Level 2 (async VideoExtractWorker: yt-dlp subtitle → ASR fallback → enriched content). Relevance rules stored in DB, managed via admin UI. ASR config as separate unit. LLM primary+backup.
...
**New files:**
- `services/policy-crawler/migrations/023_relevance_video.sql` — relevance_rules, relevance_thresholds, video_extract_status, seed data
- `services/policy-crawler/migrations/024_asr_llm_backup.sql` — asr_configs table, LLM backup columns
- `services/policy-crawler/internal/crawler/relevance.go` — RelevanceFilter, Rule, Score()
- `services/policy-crawler/internal/crawler/video_extract.go` — VideoExtractWorker, task queue, pipeline
- `services/policy-crawler/internal/crawler/asr.go` — ASRProvider interface + volcengine impl
- `services/policy-crawler/internal/admin/admin_relevance.go` — relevance rules admin API
- `services/policy-crawler/internal/admin/admin_asr.go` — ASR config admin API

**Modified files:**
- `services/policy-crawler/internal/crawler/crawler.go` — CrawlResult: add VideoURL, NeedsVideoExtract, ContentType
- `services/policy-crawler/internal/crawler/douyin_crawler.go` — strict author discovery, relevance pre-filter, set NeedsVideoExtract
- `services/policy-crawler/internal/crawler/wechat_crawler.go` — mixed mode, __biz discovery, anti-bot mitigation
- `services/policy-crawler/internal/crawler/manager.go` — VideoExtractQueue init, recovery, crawlAndProcess dispatch
- `services/policy-crawler/internal/crawler/store.go` — SaveRawTextReturningID, video_extract_status CRUD, GetPendingVideoExtracts, GetUnprocessedRawTexts update
- `services/policy-crawler/internal/admin/admin_llm.go` — LLMConfig: add backup fields
- `services/policy-crawler/internal/admin/admin_page.go` — new "相关性规则" tab, ASR section, test panel
- `services/policy-crawler/internal/llm/llm.go` — Client: add backup config, fallback logic
- `services/policy-crawler/cmd/main.go` — init worker, routes, recovery
- `services/policy-crawler/Dockerfile` — add python3, ffmpeg, yt-dlp
...
func (w *VideoExtractWorker) process(task VideoExtractTask) {
	...
	if task.RetryCount == 0 {
		...
		if desc != "" {
			enriched = fmt.Sprintf("【标题】%s\n【描述】%s\n【视频转录】%s", parts[0], desc, transcript)
		}
	}
...
Run: `go build ./...` in `services/policy-crawler`
Expected: compiles (may need `m.filter` field on CrawlerManager — add `filter *RelevanceFilter` to struct, initialized to nil for now, will be wired in Task 9)
...
- `RelevanceRulesListHandler(store)` — GET, list rules with optional category/scope filter
- `RelevanceRulesCreateHandler(store)` — POST, create rule
- `RelevanceRulesUpdateHandler(store)` — PUT, update rule (weight, enabled, scope)
- `RelevanceRulesDeleteHandler(store)` — DELETE, delete rule
- `RelevanceThresholdGetHandler(store)` — GET `/admin/relevance/thresholds/{source_id}`
- `RelevanceThresholdSetHandler(store)` — PUT `/admin/relevance/thresholds/{source_id}`
- `RelevanceTestHandler(filter)` — POST, test text score
- `RelevanceBulkImportHandler(store)` — POST, bulk import rules
...
- `ASRConfigGetHandler(db)` — GET current config
- `ASRConfigSaveHandler(db)` — POST update config
...
### Task 13: Admin UI — Relevance Rules + ASR + LLM Backup

**Files:**
- Modify: `services/policy-crawler/internal/admin/admin_page.go`

- [ ] **Step 1: Add "相关性规则" tab**
...
4. Test panel (input text → show score + matched keywords)
...
```powershell
docker exec nsi-policy-crawler wget -qO- --post-data '{"source_id":"DOUYIN-上海社保规划找Vicky"}' --header='Content-Type: application/json' http://admin:changeme@localhost:39403/admin/sources/crawl
```
