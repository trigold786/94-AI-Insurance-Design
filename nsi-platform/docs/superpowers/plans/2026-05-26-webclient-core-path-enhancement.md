...
**Files:** None new — just build and deploy
...
**Files:**
- Modify: `services/api-server/internal/handler/webclient_handler.go` — the `webClientHTML` const
...
**Files:**
- Modify: `services/api-server/internal/handler/webclient_handler.go` — the JS in `webClientHTML`
...
function retireTimeline(dob,gender,origAge){
  ...
  if(delayPct>0)h+='<div class="timeline-segment" style="width:'+delayPct+'%;background:#F59E0B">延迟 +'+(ri.years-oldRetireYear)+'年'+ri.months+'月</div>';
  h+='</div>';
  h+='<div class="timeline-labels"><span>'+by+'年 (出生)</span><span>'+oldRetireYear+'年 (原法定)</span><span>'+newRetireYear+'年'+ri.months+'月 (实际退休)</span></div>';
  if(newRetireYear>oldRetireYear){
    h+='<div style="margin-top:6px;font-size:12px;color:#D97706;background:#FEF3C7;padding:6px 10px;border-radius:6px">2025延迟退休新政：延迟 '+(ri.years-oldRetireYear)+'年'+(ri.months<10?'0':'')+ri.months+'月</div>';
  } else {
    h+='<div style="margin-top:6px;font-size:12px;color:#059669;background:#D1FAE5;padding:6px 10px;border-radius:6px">2025延迟退休新政：您的延迟幅度为0，退休时间不受影响</div>';
  }
  return h;
}
```
...
**Files:**
- Modify: `services/api-server/internal/handler/webclient_handler.go` — the `showPlan()` and `onGeneratePlan()` functions
...
function onGeneratePlan(){
  ...
  api('POST','/v1/plans/generate',req).then(function(plan){
    ...
    var h='<div class="alert-success mt-16" style="margin-top:12px">方案生成成功！共 '+schemes.length+' 个方案</div>';
...
**Files:**
- Modify: `services/api-server/internal/handler/webclient_handler.go` — add simulator section in plan result

- [ ] **Step 1: Add simulator HTML generation in onGeneratePlan()**

After the `h+='<</div>';` that closes the last scheme card, and before the "查看完整报告" button, insert:
...
```js
    ...
    h+='<div class="sim-slider-row"><label>缴费年限</label><input type="range" id="sim-years" min="1" max="40" step="1" value="'+Math.floor(_formData.months/12)+'" oninput="onSimChange()"><span class="sim-val" id="sim-years-val">'+Math.floor(_formData.months/12)+'年</span></div>';
...
function onSimChange(){
  ...
  document.getElementById('sim-base-val').textContent=base+'元';
  document.getElementById('sim-years-val').textContent=years+'年';
  document.getElementById('sim-budget-val').textContent=budget+'元';
...
function GetCityInfoJS(code){
  var cities={'310000':{name:'上海',avgSalary:12383,pensionRate:0.08,medicalRate:0.02},'110000':{name:'北京',avgSalary:15764,pensionRate:0.08,medicalRate:0.02},'440300':{name:'深圳',avgSalary:14530,pensionRate:0.08,medicalRate:0.02},'440100':{name:'广州',avgSalary:13795,pensionRate:0.08,medicalRate:0.02},'330100':{name:'杭州',avgSalary:9625,pensionRate:0.08,medicalRate:0.02}};
...
- [ ] **Step 5: Smoke test — generate a plan**
...
- [x] Type consistency: `SchemeResult.AfterTaxPension` (float64) → `models.Scheme.AfterTaxPension` (float64), `SchemeResult.Cashflow` ([]models.CashFlowItem) → `models.Scheme.Cashflow` ([]CashFlowItem) — types match
