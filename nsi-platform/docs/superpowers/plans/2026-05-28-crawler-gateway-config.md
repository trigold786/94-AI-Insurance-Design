...
This route has NO `BasicAuth` wrapper 鈥?it's unauthenticated for Docker-internal use only.
...
**Simpler approach:** Keep `RunExtraction` as-is (it still reads from local DB as fallback), but the primary extraction path (`LLMExtractRunHandler`) now uses the gateway client. The `RunExtraction` method is not called from any active path 鈥?it's exposed via `LLMStore` interface but only used by the now-secondary `LLMStatusHandler`.

Actually, let's verify 鈥?check if `RunExtraction` is called anywhere:

Search for callers of `RunExtraction` 鈥?if it's only used via `LLMStore` interface and only called from tests or inactive handlers, we can skip modifying it for now. The primary extraction path is `LLMExtractRunHandler`.
...
```html
<div class="card">
  <div class="card-header">妯″瀷閰嶇疆</div>
  <p style="margin:8px 0;color:#6B7280;font-size:13px">
    LLM銆丒mbedding 鍜?ASR 妯″瀷閰嶇疆宸茬粺涓�杩佺Щ鍒?LLM Gateway 绠＄悊鍚庡彴銆?  </p>
...
- `/admin/asr/test` (tests ASR 鈥?but it needs DB access, may need updating later)
...
Open `http://localhost:39403/admin` in browser. Navigate to the "AI鎻愬彇" panel. It should show a link to the llm-gateway admin UI instead of the old config form.
