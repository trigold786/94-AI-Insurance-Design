...
**Date**: 2026-05-27
**Scope**: Policy-crawler service — crawling infrastructure improvements
...
### Architecture
```
fetchURL() → HTTP GET → check Content-Type header
  ├── text/html → existing HTML processing
  ├── application/pdf → DocParser.ParsePDF(responseBody) → plain text
  ├── application/...word... → DocParser.ParseDOCX(responseBody) → plain text
  └── other → skip (log warning)
```

### Files
- Create: `internal/crawler/doc_parser.go`
- Modify: `internal/crawler/govsite.go` — `isPolicyLink()`, `fetchURL()`
...
- Exponential backoff: 3 retries, delays 1s → 4s → 16s
...
- Modify: `internal/crawler/govsite.go` — use new client
- Modify: `internal/crawler/rss_crawler.go` — use new client
- Modify: `internal/crawler/wechat_crawler.go` — use new client
...
- New columns on `policy_sources`:
  - `request_delay_ms INT DEFAULT 1000` — delay between requests in milliseconds
  - `max_concurrent INT DEFAULT 1` — max concurrent requests per source
  - `respect_robots BOOLEAN DEFAULT true` — check robots.txt before crawling
- New file `internal/crawler/robots.go` — fetch and parse robots.txt, check Allow/Disallow rules
...
Manager.crawlAndProcess(source)
  → read source.RequestDelayMs, source.RespectRobots, source.ProxyURL
  → if RespectRobots: robots.Check(sourceURL, userAgent) → skip if disallowed
  → fetch with delay between requests
  → use ResilientHTTPClient with proxy
```

### Files
- Create: `internal/crawler/robots.go`
- Modify: `internal/crawler/manager.go` — read ethics config, enforce delays
- Modify: `internal/admin/admin_sources.go` — new fields in source update/create
- Modify: `internal/admin/admin_page.go` — source editor UI fields
...
| File | Change |
|------|--------|
| `internal/crawler/doc_parser.go` | NEW — PDF/DOCX text extraction |
| `internal/crawler/http_client.go` | NEW — resilient HTTP client with retry + proxy |
| `internal/crawler/robots.go` | NEW — robots.txt parser and checker |
